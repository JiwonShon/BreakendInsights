...
.BDNregionBED <- function(DataPath, aa_barcode, outputpath, band_width) {
  ...
  if (!is_empty(CycleFiles)) {
    ...
    Combined_Discordant_gr = GRanges(seqnames=NULL,ranges=NULL,strand=NULL)
    ...
    for (j in 1:n_amplicon) {
      ...
      #### ver2: add this block to extract missing discordant seg to version2 ####
      Discordant_segs <- qq[substr(qq, 1, nchar("discordant")) == "discordant"]
      if (length(Discordant_segs) > 0) {  # discordant line이 있는 경우에만 처리
        Discordant_segs <- ldply(Discordant_segs, function(x) {
          xx <- unlist(strsplit(x, split = "\t"))
          coords <- unlist(strsplit(xx[2], "->"))
          chr1 <- unlist(strsplit(coords[1], ":"))[1]
          start <- as.integer(sub("[+-]$", "", unlist(strsplit(coords[1], ":"))[2]))
          chr2 <- unlist(strsplit(coords[2], ":"))[1]
          end <- as.integer(sub("[+-]$", "", unlist(strsplit(coords[2], ":"))[2]))
          data.frame(
            chr = c(chr1, chr2),
            start = c(start-half_width, end-half_width),
            end = c(start+half_width, end+half_width),
            stringsAsFactors = FALSE
          )
        })
        print("Discordant_segs:")
        print(Discordant_segs)

        Discordant_gr <- as(Discordant_segs[, c(1:3)], "GRanges")
        print("Discordant_gr:")
        print(Discordant_gr)

        Combined_Discordant_gr <- plyranges::bind_ranges(Combined_Discordant_gr, Discordant_gr)  # 병합

      } else { 

        # Discordant_gr = GRanges(seqnames=NULL,ranges=NULL,strand=NULL)
        # Combined_Discordant_gr <- plyranges::bind_ranges(Combined_Discordant_gr, Discordant_gr)  # 병합
        print("No discordant lines found.")
      }
      # print("Combined_Discordant_gr:")
      # print(Combined_Discordant_gr)
      #### ver2.
      ...
      rm(Cycle_segs,Graph_segs,Intervals, pp, qq, Discordant_segs)
  ...
  seqlevelsStyle(Combined_Discordant_gr) = "UCSC"
  ...
  BND_bed = reduce_ranges(plyranges::bind_ranges(startFlank, endFlank, Combined_Discordant_gr))
